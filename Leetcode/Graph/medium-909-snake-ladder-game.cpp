...
public:
    int snakesAndLadders(vector<vector<int>>& board) {
        ...
        //Fill the board respective cell mapping with (row,col)
        for(int i=n-1; i>=0; i--){
            for(int& j : col){
                cells[cell++] = {i, j};
            }
        ...
        while(!q.empty()){
            cur = q.front();
            q.pop();
            for(int k=cur+1; k<=min(6+cur, n2); k++){
                i = cells[k].first;
                j = cells[k].second;
                next = board[i][j] == -1 ? k : board[i][j];
